...
Get-WindowsUpdate -Install -UpdateType Driver -AcceptAll -IgnoreReboot -ErrorAction SilentlyContinue | Select Title, KB, Result | Format-Table
Get-WindowsUpdate -Install -IgnoreUserInput -AcceptAll -IgnoreReboot -MicrosoftUpdate -NotCategory 'FeaturePacks' -NotTitle 'Preview' -ErrorAction SilentlyContinue | Select Title, KB, Result | Format-Table
